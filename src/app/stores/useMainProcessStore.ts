...
const useMainProcessStore = create<mainProcessStore>((set, get) => ({
    ...
    setOptionArr: (id: string, type: string, title: string) => {
        ...
        if (type === 'M') {
            ...
            if (mCount >= 2) { alert('최대 2개까지 선택 가능합니다.'); return; }
        }
        if (type === 'G') {
            const gCount = optionArr.filter(item => item.type === 'G').length;
            if (gCount >= 3) { alert('최대 3개까지 선택 가능합니다.'); return; }
        }
        set((state) => ({ optionArr: [...state.optionArr, { id, type, title }] }));
    },
    removeOptionArr: (id: string) => {
        set((state) => ({ optionArr: state.optionArr.filter((arr) => arr.id !== id) }));
    },
    selectPersonnel: (id: string, type: string, title: string) => {
        set(() => ({ optionArr: [{ id, type, title }] }));
    },
    optionClean: (step: number) => {
        set({ process: step, optionArr: [], platform: 'ALL' });
    },
    isLoading: false,
